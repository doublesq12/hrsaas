<template>
  <div class="dashboard-container">
        ...
        <el-table border :data="employees">
          ...
          <el-table-column label="操作" sortable="" fixed="right" width="280">
            <template slot-scope="{ row }">
              ...
              <el-button type="text" size="small" @click="onRemove(row.id)" v-if="isHas(point.employees.del)"
                >删除</el-button
              >
            </template>
          </el-table-column>
    ...
    />
    <el-dialog :visible.sync="ercodeDialog" title="头像二维码">
      <canvas id="canvans"></canvas>
    </el-dialog>
    <assign-role :visible.sync="showAssignRole" :employeesId="currentEmployeesId"/>
  </div>
</template>

<script>
import { getEmployeesInfoApi } from '@/api/departments'
...
export default {
  ...
  methods: {
    ...
    showErCodeDialog(staffPhoto){
      ...
      this.$nextTick(()=>{
        const canvas=document.getElementById('canvans')
